...
case_plot2 <- function(df,testing = FALSE, facet = "wrap", nrow = NULL,
                      gridvar = NULL)
{
  ...
  inner_plot <- function(toplot){
    ggplot(toplot, aes(x = week,y = med_cases,col = intervention, fill = intervention))+
      geom_line()+
      geom_ribbon(aes(ymax = Q75,ymin = Q25),alpha = 0.1,linetype = 0)+
      theme_ls()+
      scale_colour_manual(values = c("indianred1",
  ...
  if(facet == "wrap"){
    p <- inner_plot(toplot) + facet_wrap(~type,nrow = nrow, scales = "free")
  } else {
    if(facet == "grid"){
      p <- inner_plot(toplot) + facet_grid(type~newvar, scales = "free")
    } else {
      p <- inner_plot(toplot)
    }
  }
...
#' @examples
#'
dist1b_func<-function(am,returns,dist.prop){
  am.i<-graph_from_adjacency_matrix(am,"undirected",T)
  ews<-E(am.i)$weight
  d1<-which(ews==1)
  nd1<-length(d1)
...
#' Simulate social distancing using data across days no reassignment
#'
#' @author Josh Firth
#' @param am association matrix network
#' @param returns format for output - either "graph", "matrix" or "edgelist"
...
dist2b_func<-function(am,am.contacts,returns,dist.prop){
  am.i<-graph_from_adjacency_matrix(am,"undirected",T)
  ews<-E(am.i)$weight
  d1<-which(ews==1)
  nd1<-length(d1)
